SELECT 
	CONCAT(Richieste.NumRichiesta, '-', Richieste.AnnoRichiesta)  AS RicTrapN
	,Richieste.DataInizioP
	,left(convert(nvarchar(15), Richieste.DataInizioP, 114), 5) AS OraInizioPrev
	,Ordini.DataPresaInCarico
	,Ordini.IDRisorsa
	,Risorse.NOME AS Operatore
	,CONCAT(edif1.EdificioBreve, ' - ', desusopiani1.Destinazione, ' - ', desusoamb1.Destinazione, ' - ',  amb1.CodiceAssAmbiente)  AS LuogoPrelievo
	,reparti1.Nome AS RepartoPrelievo,  CONCAT(edif2.EdificioBreve, ' - ', desusopiani2.Destinazione, ' - ', desusoamb2.Destinazione, ' - ',  amb2.CodiceAssAmbiente)  AS LuogoDestinazione
	,reparti2.Nome AS RepartoDestinazione
	,Richieste.IdRich
	,StatiOrdini.IDStato
	,Ordini.IdOrd
...
AND amb2.IdDestinazioneduso = desusoamb2.IdDestinazioneuso
	WHERE     (tblMestieri.TPL_TrasportoPazientiELogistica = 1)
		...
		and Ordini.DataPresaInCarico is null
